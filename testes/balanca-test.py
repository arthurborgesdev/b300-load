...
class HX711:
    ...
    def read(self):
        ...
        dataBits = [createBoolList(), createBoolList(), createBoolList()] # [MSB0, MSB1, MSB2] 
        dataBytes = [0x0] * 4 # create [0, 0, 0, 0]

        for j in range(2, -1, -1):  # Starts from MSB2, then MSB1, then MSB0
            for i in range(0, 8): # Fill MSB2 with inputs from HX711
                GPIO.output(self.PD_SCK, True)
                dataBits[j][i] = GPIO.input(self.DOUT)
                GPIO.output(self.PD_SCK, False)
            dataBytes[j] = numpy.packbits(numpy.uint8(dataBits[j])) # Convert MSB2 to uint8 and pack into a 8 bit array (with 0s after)
